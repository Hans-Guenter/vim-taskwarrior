...
function! taskwarrior#sort#by_column(polarity, column)
    ...
    " let default  = system(g:tw_cmd.' _get -- rc.report.'.b:command.'.sort')[0:-2]
    let default  = matchstr(system(g:tw_cmd.' show | grep report.'.b:command.'.sort')[0:-2], '\S*$')
...
function! taskwarrior#sort#order_list()
    let fromrc = matchstr(b:rc, 'rc\.report\.'.b:command.'\.sort.\zs\S*')
    if fromrc == ''
        " let list = split(system(g:tw_cmd.' _get -- rc.report.'.b:command.'.sort')[0:-2], ',')
